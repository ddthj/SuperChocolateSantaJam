...
class Entity:
    ...
    def render(self):
        # unused. The camera is responsible for rendering entities.
        pass
...
class Spawner(Entity):
    ...
    def on_collide(self, collision, other):
        other.spawnpoint = self.location.copy()
        self.hitbox.color = (0, 200, 200)
...
class Player(Entity):
    def __init__(self, **kwargs):
        ...
        self.double_jump = 0
        self.last_gap = 0
        self.spawnpoint = self.location.copy()

    def on_collide(self, collision, other):
        if other.name == "builder":
            self.on_ground = True
            self.last_gap = collision.gap
